        ...
        <div className="mx-auto max-w-[1180px]">
          {posts.length === 0 ? (
            ...
            <div className="grid gap-7 sm:grid-cols-2 lg:grid-cols-3">
              {posts.map((p) => (
                <article key={p.id} className="overflow-hidden rounded-[20px] border border-[var(--color-line)] bg-white shadow-[0_20px_44px_-28px_rgba(20,40,20,0.4)] transition hover:-translate-y-1">
                  <Link href={`/blog/${p.slug}`} className="group block no-underline">
                    <div className="relative h-[200px] w-full overflow-hidden bg-[#e8e4d4]">
                      {p.coverImage && <Image src={p.coverImage} alt={p.title} fill className="object-cover transition group-hover:scale-105" />}
                    </div>
                    <div className="p-7">
                      <div className="mb-2.5 text-[12px] uppercase tracking-[0.16em] text-olive">{p.category}</div>
                      <h2 className="m-0 font-serif text-[24px] font-semibold leading-[1.15] text-green-deep">{p.title}</h2>
                      <p className="mt-3 line-clamp-3 text-[15px] font-light leading-[1.6] text-muted">{p.excerpt}</p>
                      <div className="mt-4 text-[12.5px] text-muted">{formatDate(p.publishedAt)} · {p.readMinutes} min read</div>
                    </div>
                  </Link>
                </article>
              ))}
